...
test.describe("time keeper page", () => {
  ...
  test("displays correct header and information text", async ({ page }) => {
    await expect(page.getByRole('heading', { name: 'Time Keeper' })).toBeVisible();
    await expect(page.getByText('This app helps you keep track of your friends’ timezones!', { exact: true })).toBeVisible();
  });
  ...
  test("displays local time on page load", async ({ page }) => {
    ...
    await expect(page.getByRole('cell', { name: getLocalTime(locale) })).toBeVisible();
  });

  timezoneRecords.forEach((record) => {
    test(`allows user to add a new record for ${record.location} timezone and displays correct time`, async ({ page }) => {
      ...
      await expect(page.getByText(record.location, { exact: true })).toBeVisible();
  ...
  test("allows user to delete existing records", async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Save' }).click();
    await page.getByRole('button', { name: 'Delete , John Doe - New York' }).click();
    await expect(page.locator('text=John Doe - New York').last()).not.toBeVisible();
  });

  // skipping this test as it is a known issue
  test.skip("displays records sorted in ascending order by local time", async ({ page }) => {
    const locale = Intl.DateTimeFormat().resolvedOptions().timeZone;
    await page.getByRole('button', { name: 'Add timezone' }).click();
    await page.getByPlaceholder('Label').click();
    ...
    await page.getByRole('button', { name: 'Save' }).click();
    await page.getByRole('button', { name: 'Add timezone' }).click();
    const labelArray = await page.locator('tbody tr td:nth-child(1)').allTextContents()
  ...
  // skipping this test as it is a known issue
  test.skip("does not allow user to delete local timezone records", async ({ page }) => {
    await expect(page.locator('tbody tr:nth-of-type(1) td:nth-child(1)')).toBeDisabled();
  });
});